...
type Todo = z.infer<typeof createTodoSchema> & { id: string }
...
const TodoProvider = ({ children }: TodoProviderProps) => {
  ...
  const storedTodosData = useMemo(() => localStorage.getItem('todos'), [])
  ...
  const add = useCallback(({ title, description }: Omit<Todo, 'id'>) => {
    const newTodo = {
      id: uuidv4(),
      title: title,
